...
function requestInteger(message, min, max) {
    /*
     * boucle qui s'exécute tant que l'entier n'est pas un nombre (fonction isNaN()) et
     * n'est pas compris entre les arguments min et max -> [min,max].
     */

    do {
        message = parseInt(prompt("Veuillez choisir un niveau de difficulté compris entre 1 et 3 :"))
    } while (isNaN(message) && isNaN(min) >= message && message >= isNaN(max))
...
function getRandomInteger(min, max) {
    //retourne un chiffre au hasard entre le minimum et le max
...
function throwDices(dices, sides) {
    ...
    for (index = 0; index < dices; index++) {
        sum += getRandomInteger(1, parseInt(sides))
    }
    return sum;
    /*
